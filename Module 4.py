...
## Radar Chart
# 1. Compute average feature values for each cluster
cluster_means = airport_df.groupby('Cluster')[features].mean()

# 2. Normalize the data to [0, 1] for fair visual comparison
cluster_means_norm = (cluster_means - cluster_means.mean()) / cluster_means.std()

# 3. Setup radar plot structure
labels = features
num_vars = len(labels)
